...
if (-not $password) { throw "password not found in .env" }

$outFull = (Resolve-Path -LiteralPath (Split-Path -Parent $OutputPath) -ErrorAction SilentlyContinue)
if (-not $outFull) {
  New-Item -ItemType Directory -Force -Path (Split-Path -Parent $OutputPath) | Out-Null
}
$outFullPath = (Resolve-Path -LiteralPath $OutputPath -ErrorAction SilentlyContinue)
if (-not $outFullPath) {
  $outFullPath = (Resolve-Path -LiteralPath (Split-Path -Parent $OutputPath)).Path + "\\" + (Split-Path -Leaf $OutputPath)
}

$env:PGPASSWORD = $password
try {
  & psql -h $dbHost -p $dbPort -U $dbUser -d $dbName -X -q --csv -v ON_ERROR_STOP=1 -c $Query | Set-Content -Encoding UTF8 -Path $outFullPath
