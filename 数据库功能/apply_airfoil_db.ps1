...
$sqlDir = Join-Path $Root "数据库功能\\sql"
...
$sqlFiles = @(
  ...
  "01_views.sql",
  "04_query_functions.sql",
  "05_airfoil_db_governance.sql",
  "06_airfoil_db_import_function.sql",
  "03_anomaly_conflict_index.sql",
  "02_functions_seed.sql"
...
try {
  foreach ($file in $sqlFiles) {
    ...
    & psql -h $dbHost -p $dbPort -U $dbUser -d $dbName --single-transaction -v ON_ERROR_STOP=1 -f $file | Out-Host
