...
$sqlFile = Join-Path $Root "数据库功能\\sql\\10_migrate_public_to_airfoil_db.sql"
...
try {
  & psql -h $dbHost -p $dbPort -U $dbUser -d $dbName --single-transaction -v ON_ERROR_STOP=1 -f $sqlFile | Out-Host
