...
## 🔬 §7 数据库高级机制实验验证 — `TestLab/`

基于项目文档 §7 要求，在 `TestLab/` 目录下完成了三类实验的全过程验证。

### 7.1 索引设计与性能实验

围绕高频复杂查询 **Q2（工况筛选）** 和 **Q5（异常识别）** 进行三阶段对比：

| 阶段 | Q2 执行耗时 | Q5 执行耗时 | 索引策略 |
|:-----|:-----------|:-----------|:---------|
| Phase 0 — 无索引基准 | 3.079 ms | 3.849 ms | 仅主键 |
| Phase 1 — 单列索引 | 0.618 ms (**5.0x**) | 1.567 ms (**2.5x**) | 单列 B-Tree |
| Phase 2 — 复合/部分索引 | 0.579 ms (**5.3x**) | 1.236 ms (**3.1x**) | 复合索引 + 部分索引 |

**实验提交物**：查询语句 \| EXPLAIN ANALYZE 执行计划文本 \| 性能对比分析 \| 索引选择理由

### 7.2 事务与并发实验

| 场景 | 类型 | 验证结果 |
|:-----|:-----|:---------|
| 批量导入回滚 | 事务原子性 | 非法 Cd < 0 触发整批回滚，行数不变 ✅ |
| 乐观锁冲突 | 并发控制 | 过期 xmin 更新被拒绝，版本冲突检测生效 ✅ |
| 悲观锁 | 行级锁 | SELECT FOR UPDATE 排他等待，超时机制生效 ✅ |

### 7.3 视图、触发器与存储过程

| 类型 | 对象名 | 功能 |
|:-----|:-------|:-----|
| 视图 | `v_current_airfoil_version` | 统一展示当前有效版本 |
| 视图 | `v_data_source_summary` | 数据来源统计（real vs synthetic） |
| 视图 | `v_performance_version_trace` | 性能数据版本追溯 |
| 触发器 | `trg_log_change_*` | 自动写入修改审计日志 |
| 触发器 | `trg_prevent_core_delete` | 阻止核心数据物理删除（软删除强制） |
| 存储过程 | `import_performance_batch` | 批量导入 + 合法性检查 + 事务回滚 |
| 存储过程 | `update_performance_record_*` | 乐观锁/悲观锁并发更新 |

**覆盖率**：§7.1 (8/8) + §7.2 (4/4) + §7.3 (9/9) = **21/21 100%**

> 详细报告见 [document/](file:///c:/Users/ASUS/Desktop/Databaselab/Bigwork/AEDS/document/) 下的 PDF 及 [TestLab/](file:///c:/Users/ASUS/Desktop/Databaselab/Bigwork/AEDS/TestLab/) 下的 SQL 源码和 Markdown 报告。
...
│
├── TestLab/                     # [§7 数据库高级机制实验]
│   ├── 01_index_experiment.sql       # 索引优化实验（3 阶段对比）
│   ├── 02_transaction_experiment.sql # 事务与并发实验
│   ├── 03_advanced_mechanisms.sql    # 视图/触发器/存储过程验证
│   ├── navicat_*.sql                 # Navicat 兼容副本（无 psql 元命令）
│   ├── report_index_experiment.md    # 索引实验详细报告
│   ├── report_transaction_experiment.md
│   ├── report_advanced_mechanisms.md
│   ├── 实验覆盖率检查清单.md         # §7 全项 21/21 覆盖核对表
│   └── *_output.txt                  # 原始 EXPLAIN ANALYZE 输出
│
├── document/                    # [文档]
│   ├── Webfront架构分析与改进方案.md
│   ├── Webfront前端升级更新日志.md
│   ├── 项目2-面向数据智能协同的翼型工程数据库系统设计与实现.pdf
│   ├── AEDS_Database_Advanced_Mechanisms_Report.pdf     # §7 综合实验报告
│   ├── AEDS_Final_Consolidated_Experiment_Report.pdf    # 最终汇总实验报告
│   ├── Experiment_Report_Source/                        # LaTeX 源码包
│   │   ├── consolidated_report.tex
│   │   └── main.tex
│   └── plans/
│
...
| 异常标记 | 206 |
| Web 页面 | 11 个模板 |
| 可视化图表 | 13 张 ECharts（含 Drag Polar + 雷达图）+ 6 张 Matplotlib |
